...
func escapeGreaterUnicodeToBuffByUTF16(r rune, buf Buffer) {
	if r <= '\uffff' {
...
func escapeGreaterUnicodeToBuffByUTF8(r rune, buf Buffer) {
...
func escapeNothing(b byte, buf Buffer) {
	buf.WriteByte(b)
}

func escAsciiControlChar(b byte, buf Buffer) {
	upper := b >> 4
	lower := b & 0x0F
...
func escDoubleQuote(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', '"'})
}

func escSlash(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', '/'})
}

func escBackslash(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', '\\'})
}

func escBackspace(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 'b'})
}

func escVertTab(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 'f'})
}

func escTab(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 't'})
}

func escNewLine(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 'n'})
}

func escReturn(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 'r'})
}

func escLeftAngle(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 'u', '0', '0', '3', 'C'})
}

func escRightAngle(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 'u', '0', '0', '3', 'E'})
}

func escAnd(_ byte, buf Buffer) {
	buf.Write([]byte{'\\', 'u', '0', '0', '2', '6'})
}

// func escPercent(_ byte, buf Buffer) {
// 	buf.Write([]byte{'\\', 'u', '0', '0', '2', '5'})
// }

func escapeStringToBuff(s string, buf Buffer, opt *Opt) {
	for _, r := range s {
		if r <= 0x7F {
			b := byte(r)
