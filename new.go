...
func NewArray() *V {
	return newArray(globalValuePool{})
}
...
func NewFloat64(f float64) *V {
	...
	return newFloat64f(globalValuePool{}, f, format, -1, 64)
}
...
func NewFloat64f(f float64, format byte, prec int) *V {
	...
	return newFloat64f(globalValuePool{}, f, format, prec, 64)
}
...
func NewFloat32(f float32) *V {
	...
	return newFloat64f(globalValuePool{}, f64, format, -1, 32)
}
...
func NewFloat32f(f float32, format byte, prec int) *V {
	...
	return newFloat64f(globalValuePool{}, float64(f), format, prec, 64)
}

// -------- internal functions --------

func new(p valuePool, t ValueType) *V {
	v := valuePool.get(p)
	v.valueType = t
	return v
}

func newObject(p valuePool) *V {
	v := new(p, Object)
...
func newArray(p valuePool) *V {
	v := new(p, Array)
	return v
}

func newFloat64f(p valuePool, f float64, format byte, prec, bitsize int) *V {
	v := new(p, Number)
	// v.num = &num{}
	v.num.negative = f < 0
	v.num.f64 = f
